...
import { createPost, createPostFB } from "../redux/modules/post";
...
const Post = (props) => {
  ...
  const addPost = () => {
    ...
    const post = {
      ...
      author: author_ref.current.value, // input에서 text값에 접근하려면 ~~.value로 접근해요. (word_ref.current가 input이에요.)
  ...
  return (
    <React.Fragment>
      ...
      <InputWrapper>
        <input placeholder="글쓴이을 입력하세요" ref={author_ref} />
      </InputWrapper>
      <InputWrapper>
        <input rows="7" placeholder="내용을 입력하세요" ref={comment_ref} />
      </InputWrapper>
